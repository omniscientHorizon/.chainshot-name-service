...
contract Domains is ERC721URIStorage {
    ...
    mapping(string => string) public imageLinks;
    ...
    constructor(string memory _tld)
        payable
        ERC721("Chainshot Name Service", "CNS")
    {
    ...
    function register(string calldata name) public payable {
        ...
        // checking if u have paid enough or not
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Chainshot name service", "image": "data:image/svg+xml;base64,',
    ...
    function setImage(string calldata name, string calldata link) public {
        if (msg.sender != domains[name]) revert Unauthorized();
    ...
    function setBio(string calldata name, string calldata aboutMe) public {
        if (msg.sender != domains[name]) revert Unauthorized();
        bio[name] = aboutMe;
    }

    function getBio(string calldata name) public view returns (string memory) {
        return bio[name];
    }

    function setUserName(string calldata name, string calldata user) public {
        if (msg.sender != domains[name]) revert Unauthorized();
        userName[name] = user;
    }

    function getUserName(string calldata name)
        public
        view
        returns (string memory)
    {
